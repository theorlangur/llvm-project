...
ClangdServer::ClangdServer(const GlobalCompilationDatabase &CDB,
  ...
  std::unique_ptr<ParsingCallbacks> ParseCB = std::make_unique<UpdateIndexCallbacks>(DynamicIdx.get(), Callbacks, TFS, 
  IndexTasks ? &*IndexTasks : nullptr, PublishInactiveRegions);
...
std::optional<ParsedAST> ClangdServer::buildAST(PathRef File, StringRef Contents)
{
  class LocalStringBuffer : public llvm::MemoryBuffer {
    const StringRef BufferContents;
  ...
  public:
    ...
    LocalStringBuffer(StringRef Data, StringRef Name)
        : BufferContents(std::move(Data)), Name(Name) {
      MemoryBuffer::init(BufferContents.begin(),
                         BufferContents.end(),
                         /*RequiresNullTerminator=*/true);
    }
  };
  std::string ActualVersion = "0";
  ParseOptions Opts;

  auto DraftVFS = DraftMgr.asVFSInMem();
  DraftVFS->addFile(File, {}, std::make_unique<LocalStringBuffer>(Contents, File));
  ...
  Inputs.DraftFS = DraftVFS;
  {
    ...
    if (Cmd)
      ...
      Inputs.CompileCommand = CDB.getFallbackCommand(File);
  }
  return WorkScheduler->buildAST(File, Inputs);
}
...
void ClangdServer::codeAction(const CodeActionInputs &Params,
  ...
  auto Action = [Params, CB = std::move(CB),
                 FeatureModules(this->FeatureModules), Server = this](
