...
getFunctionSourceCode(const FunctionDecl *FD, const DeclContext *TargetContext,
  ...
  auto DelAttr = [&](const Attr *A) {
    ...
    if (!AttrTokens || AttrTokens->empty()) {
      Errors = llvm::joinErrors(
          std::move(Errors), error("create outline: Can't create out of line as "
...
///   #include "a.h"
///   void foo() { return; }
class CreateOutline : public Tweak {
public:
  ...
  std::string title() const override {
    return "Create a function body out-of-line";
  }
  ...
  bool prepare(const Selection &Sel) override {
    ...
    if (!Source 
        ...
        || Source->isDefaulted() 
        || Source->isDeleted()
        || Source->isPureVirtual()
      )
      return false;
    bool DefinitionExists = false;
    LookupRequest Req;
    Req.IDs.insert(getSymbolID(Source));
  ...
  Expected<Effect> apply(const Selection &Sel) override {
    ...
    if (Source->isInlineSpecified()) {
      ...
      HeaderUpdates = *DelInline;
    }

    if (!HeaderUpdates.empty())
    {
      if (SameFile) {
